...
export const buttonVariants = cva(
  "group inline-flex items-center justify-center rounded-sm text-sm font-medium transition-all duration-100 focus-visible:ring-1 focus-visible:ring-ring focus-visible:outline-hidden disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      look: {
        primary:
          "border-base bg-primary text-primary-foreground shadow-sm hover:brightness-95 active:press active:shadow-base",
        secondary:
          "border-base bg-secondary text-secondary-foreground shadow-sm hover:brightness-95 active:press active:shadow-base",
        alert:
          "border-base bg-alert text-alert-foreground shadow-sm hover:brightness-95 active:press active:shadow-base",
        outline:
          "border bg-background text-foreground shadow-sm hover:brightness-95 active:press active:shadow-base",
...
export const Button = component$<ButtonProps>(({ size, look, ...props }) => {
  ...
  return (
    <Comp {...props} class={cn(buttonVariants({ size, look }), props.class)}>
      <Slot />
    </Comp>
  );
});
